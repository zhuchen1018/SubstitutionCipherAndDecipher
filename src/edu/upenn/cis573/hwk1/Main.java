...
public class Main {
	    ...
	    if (dir.isDirectory()){
		  ...
		  if (directoryListing.length != 0) {
		    for (File file : directoryListing) {
		    	...
		    	correctRate += compare(original, decrypted, file.getName());
		    }
		    correctRate = (float) totalCorrect/totalCount;
			System.out.println();
			System.out.println("TotalCorrect " + totalCorrect + " , " +  (totalCount - totalCorrect) + " incorrect");
  ...
  private static String convertFileToString(File file){
	  ...
	  private static String encrypt(String text){
		  ...
		  map.put('A', 'o');map.put('B', 'p');map.put('C', 'q');map.put('D', 'r');
		  map.put('E', 's');map.put('F', 't');map.put('G', 'u');map.put('H', 'v');
		  map.put('I', 'w');map.put('J', 'x');map.put('K', 'y');map.put('L', 'z');
		  map.put('M', 'a');map.put('N', 'b');map.put('O', 'c');map.put('P', 'd');
		  map.put('Q', 'e');map.put('R', 'f');map.put('S', 'g');map.put('T', 'h');
		  map.put('U', 'i');map.put('V', 'j');map.put('W', 'k');map.put('X', 'l');
		  map.put('Y', 'm');map.put('Z', 'n');
	  ...
	  private static float compare(String originalText,String decryptedText, String Filename){
		  int count = 0;
		  int correctCount = 0;
		  float correctRate = 0;
		  ...
		  System.out.println(Filename + " " + correctCount + " correct " + (count - correctCount) + " incorrect" );
